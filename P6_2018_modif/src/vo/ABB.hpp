	...
	if (estadisticas){
		...
		if (nodo != NULL){
			if (clave < nodo->clave_)
				...
				nodo->colision_.setColision(1);
		}
	}
	else{
		if (nodo == NULL)
			nodo = new NodoBB<T>(clave, clave, col);
		else if (clave < nodo->clave_)
			insertarRama(nodo->izquierdo_, clave, estadisticas);
		else if (clave > nodo->clave_)
			insertarRama(nodo->derecho_, clave, estadisticas);
    ...
    while (!cola.empty()){
    	...
    	for (int c = 0; c < i; c++){
    		...
    		if (aux == NULL)
    			...
    			cout << "[" << aux->clave_ << "]" << " colisiones: " << aux->colision_.getColision();
...
//    cout << "MOSTRAR COLISIONES: " << colision_.getColision() << endl;
